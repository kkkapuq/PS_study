...
def solution(food_times, k):
    ...
    # 튜플로 각 음식별 섭취시간과 음식번호 넣어주기
    for i in range(n):
        heapq.heappush(h, (food_times[i], i+1))
        
    # 현재 음식 섭취시간이 k보다 작을때만 while문 실행
    # 현재 음식을 다 먹기까지 걸리는 시간은 (현재음식섭취시간 - 지금까지 먹은시간) * 남은음식의 갯수
    while (h[0][0] - time) * n < k:
        # 전체 시간에서 현재음식을 먹는데 걸리는 시간 빼주기
        k -= (h[0][0] - time) * n
        time += h[0][0] - time
        n -= 1
        heapq.heappop(h)
